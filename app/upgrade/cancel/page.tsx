import Link from 'next/link'

export default function UpgradeCancelPage() {
  return (
    <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center px-4">
      <div className="max-w-md w-full text-center space-y-4 bg-slate-900 border border-slate-800 rounded-2xl p-8">
        <p className="text-4xl">⚠️</p>
        <h1 className="text-2xl font-semibold">Checkout canceled</h1>
        <p className="text-slate-300 text-sm">
          Your Stripe session was closed before completing payment. You can restart checkout anytime from the upgrade
          page inside the app.
        </p>
        <Link
          href="/upgrade"
          className="inline-flex justify-center rounded-lg bg-blue-600 px-4 py-2 font-medium text-white hover:bg-blue-700"
        >
          Back to upgrade page
        </Link>
        <p className="text-xs text-slate-500">
          Contact support if you experienced an unintended cancellation.
        </p>
      </div>
    </div>
  )
}
